...
// Test wait --quiet flag exists
#[test]
fn test_wait_quiet_flag_exists() {
    ...
    assert!(
        ...
        "Wait help should mention --quiet: {}",
        combined
    );
}

// Test wait (default mode) reports activity when output is produced
#[test]
fn test_wait_reports_activity_on_output() {
    let env = TestEnv::new();
...
// Test wait (default mode) reports both activity and exit when command exits
#[test]
fn test_wait_on_command_exit() {
    let env = TestEnv::new();
...
fn test_wait_activity_clears_after_read() {
    ...
    assert!(output.is_err() || !output.unwrap().status.success(),
            "Second wait should block/timeout when no new activity");
}

// Test multiple events coalesce
#[test]
fn test_wait_multiple_events_coalesce() {
    let env = TestEnv::new();
...
// Test status (default mode) clears flag after read
#[test]
fn test_status_activity_clears_after_read() {
    ...
    // First status - should report activity: true
    let output = Command::new(interminai_bin())
        .arg("status")
        .arg("--socket")
        .arg(&daemon.socket_path)
    ...
    let stdout = String::from_utf8_lossy(&output.stdout).trim().to_string();
    assert!(stdout.contains("Activity: true"), "First status should report true: got '{}'", stdout);

    // Second status - should report activity: false (flag was cleared)
    let output = Command::new(interminai_bin())
        .arg("status")
        .arg("--socket")
        .arg(&daemon.socket_path)
    ...
    let stdout = String::from_utf8_lossy(&output.stdout).trim().to_string();
    assert!(stdout.contains("Activity: false"), "Second status should report false after clear: got '{}'", stdout);
    ...
    // Third status - should report activity: true again
    let output = Command::new(interminai_bin())
        .arg("status")
        .arg("--socket")
        .arg(&daemon.socket_path)
    ...
    let stdout = String::from_utf8_lossy(&output.stdout).trim().to_string();
    assert!(stdout.contains("Activity: true"), "Third status should report true after new output: got '{}'", stdout);
}
